...
class SiteController extends Controller
{
        ...
        if(isset($tag)) {
            ...
            if($data->total() > 0) {
                ...
                if(empty($tag->meta_title)) {
                    $tag->meta_title = $tag->name.' | donoithat.org';
                }
    ...
    public function page(Request $request, $slug)
    {
        ...
        if(isset($type)) {
            ...
            if(empty($type->meta_title)) {
                $type->meta_title = $type->name.' | donoithat.org';
            }
            ...
            if(empty($type->meta_description)) {
                $typeNameNoLatin = CommonMethod::convert_string_vi_to_en($type->name);
                $type->meta_description = $typeNameNoLatin.', '.$type->name.' - donoithat.org';
            }
            // lay ra the loai con neu the loai nay ko co bai viet
            if($total <= 0) {
                $typeChild = DB::table('post_types')
                    ->select('id', 'name', 'slug', 'parent_id', 'image', 'grid')
        ...
        if(isset($post)) {
            ...
            if(empty($post->meta_title)) {
                $post->meta_title = $post->name.' | '.$post->name.' - donoithat.org';
            }
    ...
    private function getPostTypeQuery($id, $ids)
    {
        $data = DB::table('posts')
            ...
            ->select('posts.id', 'posts.name', 'posts.slug', 'posts.image', 'posts.summary', 'posts.description', 'posts.created_at', 'posts.type', 'posts.price', 'posts.price_old', 'posts.discount')
    ...
    {
        $data = DB::table('posts')
            ->select('id', 'name', 'slug', 'image', 'summary', 'description', 'created_at', 'type', 'price', 'price_old', 'discount')
    ...
    */
    ...
    public function love(Request $request)
    {
        ...
        //find data
        $data = DB::table('posts')
                    ->select('id', 'name', 'slug', 'price', 'price_old', 'discount', 'image')
                    ->where('id', $request->id)
                    ->where('type', PRODUCT)
                    ->where('status', ACTIVE)
                    ->where('start_date', '<=', date('Y-m-d H:i:s'))
        ...
        //add to cart
        if(count($data) > 0) {
            Cart::add($data->id, $data->name, 1, !empty($data->price)?$data->price:0, ['image' => $data->image, 'slug' => $data->slug, 'price_old' => $data->price_old, 'discount' => $data->discount]);
        }
    ...
    public function unlove(Request $request)
    {
        ...
        $content = Cart::content();
        $rowId = $request->rowId;
        if(count($content) > 0 && !empty($rowId)) {
            Cart::remove($rowId);
            return view('site.common.loved')->render();
        }
        return;
    ...
    public function order(Request $request)
    {
        ...
        $checkIP = DB::table('contacts')->where('ip', $ip)->where('created_at', '>', $past)->count();
        if($checkIP > 0) {
            return 3; // da them truoc do. 60s moi cho phep tiep tuc
        }
        trimRequest($request);
        $validator = Validator::make($request->all(), [
            'id' => 'required|numeric',
        ...
        $product = DB::table('posts')
                    ...
                    ->first();
        if(!empty($product)) {
            $msg = 'Đơn đặt hàng sản phẩm <a href="' . $product->slug . '">' . $product->name . '(' . $product->id . ')</a>';
        }
        $data = Contact::create([
                'name' => $request->name,
                'email' => $request->email,
